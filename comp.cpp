...
public:
	FloatComp() : Comp(FLOAT0, sizeof(float)) {}
public:
	virtual bool eval(const void* lhs, Operator op, const void* rhs);
};
...
public:
	StrComp(int len) : Comp(STRING0, len) {}
public:
	virtual bool eval(const void* lhs, Operator op, const void* rhs);
};
...
Comp* make_comp(AttrType type, int len)
{
	switch (type)
	{
	case INT0: return new IntComp();
	case FLOAT0: return new FloatComp();
	case STRING0: return new StrComp(len);
	}
	return nullptr;
}